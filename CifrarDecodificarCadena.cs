...
namespace CifradoCadena
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Ingrese letra de inicio");
            LI = Convert.ToChar(Console.ReadLine());
            Console.WriteLine("Ingrese letra de cifrado");
            LC = Convert.ToChar(Console.ReadLine());
            Console.WriteLine("Ingrese la cadena codificada");
            cadena.palabra = Console.ReadLine() ;
            Console.WriteLine(cadena.Decodificar(LI, LC));
            Console.ReadKey();
        }
    }
    ...
    class Cadena
    {
        ...
        public string Decodificar(char LI, char LC)
        {//97 - 122
            string salida = "", aux = palabra.ToLower(); //Convierto a minisculas para trabajar desde a (97) hasta z (122)
            char dif;
            if (LC > LI) dif = (char)(LC - LI); //Calculo la diferencia de las letras inicios, ya que esa diferencia se le resta a la letra de la cadena
            else dif = (char)(LI - LC);
            for (int i = 0; i < aux.Length; i++)
            {
                if ((aux[i] - dif) < 97) //Si dicha resta de la diferencia es menor a 97 se le suma 26 para volver al final EJ la resta da 96 + 26 = 122, que corresponde a la 'z'
                    salida += (char)((aux[i] - dif) + 26);
                else //Si no sobre pasa el limite se resta normal
                    salida += (char)(aux[i] - dif);
            }
            return salida;
        }
